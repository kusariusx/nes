...
imm :: proc(cpu: ^CPU, bus: Bus, cycle: u8, action: proc(cpu: ^CPU, bus: Bus, value: u8)) {
    switch cycle {
    case 2:
...
zpg_read :: proc(cpu: ^CPU, bus: Bus, cycle: u8, action: proc(cpu: ^CPU, bus: Bus, value: u8)) {
    switch cycle {
    case 2:
    ...
    case 3:
        ...
        value := bus_read(bus, u16(cpu.instruction_operand))
...
zpgi_read :: proc(cpu: ^CPU, bus: Bus, cycle: u8, index_register: u8, action: proc(cpu: ^CPU, bus: Bus, value: u8)) {
    switch cycle {
    case 2:
        // Fetch address, increment PC
        cpu.instruction_operand = bus_read(bus, cpu.PC)
...
abs_read :: proc(cpu: ^CPU, bus: Bus, cycle: u8, action: proc(cpu: ^CPU, bus: Bus, value: u8)) {
    switch cycle {
    case 2:
    ...
    case 4:    
        // Fetch value from effective address, perform action, done
        value := bus_read(bus, cpu.instruction_operands.whole)
        action(cpu, bus, value)
        cpu_instruction_done(cpu)
    }
}
...
absi_read :: proc(cpu: ^CPU, bus: Bus, cycle: u8, index_register: u8, action: proc(cpu: ^CPU, bus: Bus, value: u8)) {
    switch cycle {
    case 2:
        // Fetch low byte of address, increment PC
        cpu.instruction_operands.bytes[LOW] = bus_read(bus, cpu.PC)
...
indx_read :: proc(cpu: ^CPU, bus: Bus, cycle: u8, action: proc(cpu: ^CPU, bus: Bus, value: u8)) {
    switch cycle {
    case 2:
    ...
    case 3:
        ...
        bus_read(bus, u16(cpu.instruction_operand)) // Performed by hardware but discarded
...
indy_read :: proc(cpu: ^CPU, bus: Bus, cycle: u8, action: proc(cpu: ^CPU, bus: Bus, value: u8)) {
    switch cycle {
    case 2:
        cpu.instruction_operand = bus_read(bus, cpu.PC)
        cpu.PC += 1
...
zpg_read_modify_write :: proc(cpu: ^CPU, bus: Bus, cycle: u8, action: proc(cpu: ^CPU, bus: Bus, value: ^u8)) {
    switch cycle {
    case 2:
        // Fetch address, increment PC
        cpu.instruction_operand = bus_read(bus, cpu.PC)
    ...
    case 3:
        // Read from effective address
        cpu.instruction_temp_value = bus_read(bus, u16(cpu.instruction_operand))
    case 4:
        // Dummy write the value back to effective address, perform action
        bus_write(bus, u16(cpu.instruction_operand), cpu.instruction_temp_value)
...
zpgi_read_modify_write :: proc(cpu: ^CPU, bus: Bus, cycle: u8, index_register: u8, action: proc(cpu: ^CPU, bus: Bus, value: ^u8)) {
    switch cycle {
    case 2:
        // Fetch address, increment PC
        cpu.instruction_operand = bus_read(bus, cpu.PC)
...
abs_read_modify_write :: proc(cpu: ^CPU, bus: Bus, cycle: u8, action: proc(cpu: ^CPU, bus: Bus, value: ^u8)) {
    switch cycle {
    case 2:
        // Fetch low byte of address, increment PC
        cpu.instruction_operands.bytes[LOW] = bus_read(bus, cpu.PC)
...
absi_read_modify_write :: proc(cpu: ^CPU, bus: Bus, cycle: u8, index_register: u8, action: proc(cpu: ^CPU, bus: Bus, value: ^u8)) {
    switch cycle {
    case 2:
        // Fetch low byte of address, increment PC
        cpu.instruction_operands.bytes[LOW] = bus_read(bus, cpu.PC)
...
rel :: proc(cpu: ^CPU, bus: Bus, cycle: u8, should_branch: bool) {
    switch cycle {
    case 2:
...
implied :: proc(cpu: ^CPU, bus: Bus, cycle: u8, action: proc(cpu: ^CPU, bus: Bus)) {
    switch cycle {
    case 2:
...
zpg_write :: proc(cpu: ^CPU, bus: Bus, cycle: u8, value: u8) {
    switch cycle {
    case 2:
        // Fetch address, increment PC
        cpu.instruction_operand = bus_read(bus, cpu.PC)
    ...
    case 3:
        ...
        bus_write(bus, u16(cpu.instruction_operand), value)
        cpu_instruction_done(cpu)
    }
}

// Zero-page indexed addressing, write instructions
zpgi_write :: proc(cpu: ^CPU, bus: Bus, cycle: u8, index_register: u8, value: u8) {
    switch cycle {
    case 2:
        // Fetch address, increment PC
        cpu.instruction_operand = bus_read(bus, cpu.PC)
...
abs_write :: proc(cpu: ^CPU, bus: Bus, cycle: u8, value: u8) {
    switch cycle {
    case 2:
        // Fetch low byte of address, increment PC
        cpu.instruction_operands.bytes[LOW] = bus_read(bus, cpu.PC)
    ...
    case 4:    
        // Write value to effective address, done
        bus_write(bus, cpu.instruction_operands.whole, value)
        cpu_instruction_done(cpu)
    }
}

// Absolute indexed addressing, write instructions
absi_write :: proc(cpu: ^CPU, bus: Bus, cycle: u8, index_register: u8, value: u8) {
    switch cycle {
    case 2:
        // Fetch low byte of address, increment PC
        cpu.instruction_operands.bytes[LOW] = bus_read(bus, cpu.PC)
...
indx_write :: proc(cpu: ^CPU, bus: Bus, cycle: u8, value: u8) {
    switch cycle {
    case 2:
...
indy_write :: proc(cpu: ^CPU, bus: Bus, cycle: u8, value: u8) {
    switch cycle {
    case 2:
        // Fetch pointer address, increment PC
        cpu.instruction_operand = bus_read(bus, cpu.PC)
